...
function App() {
  ...
  const [modalAccept, setmodalAccept] = useState(0);
  const [DeleteItemId, setDeleteItemId] = useState(-1);

  const [showModal, setshowModal] = useState(false);
  const [msgApiResult, setMsgApiResult] = useState("");
  const [ckeckIdNewProduct, setCkeckIdNewProduct] = useState(true);

  //این یک آرایه کمکی است که در متد سرچ بکار میرود
  let uniqueTags = [];
  let items = []


  // برای عوض کردن صفحه لازمه
  const navigate = useNavigate();

  //این کل دیتا رو میگیره
  ...
  //کتگوری ها رو از سرور دریافت میکنه چون برای آخرین ردیف که اضافه کردن آیتم هست لازمه
  ...
  //کوئری که کاربر میزنه رو ست میکنه
  const changeHandler = (event) => {
    setQuery(event.target.value);
  };

  //اینو برای سرچ گذاشتم که بعد از تموم شدن تایپ کاربر 1 ثانیه بعدش بیاد سرچ کنه
  const debouncedChangeHandler = useCallback(debounce(changeHandler, 1000), []);


  //این متد آیتم با آیدی xرو پاک میکنه
  const deleteProduct = async (x) => {


    try {
      const response = await fetch("https://dummyjson.com/products/" + x, { // درخواست DELETE به سرور
        method: "DELETE"
      })

      setData(Data.filter(item => item.id !== x)) // حذف کردن محصول از لیست نمایش داده شده
      if (response.status === 200) { // بررسی وضعیت پاسخ
        setMsgApiResult("عملیات حذف با موفقیت انجام شد.") // تنظیم پیام نتیجه api
        document.getElementById("errorWindow").style.backgroundColor = "#4cbb17" // تنظیم رنگ پس زمینه پنجره خطا به سبز
        document.getElementById("errorWindow").style.display = "flex" // نمایش پنجره خطا برای 6 ثانیه
        ...
        document.getElementById("errorWindow").style.backgroundColor = "#ed4337" // تنظیم رنگ پس زمینه پنجره خطا به قرمز
        ...
        setmodalBody(response) // در صورت بروز خطا، در کادر بدنه مدال پیام خطا نمایش داده میشود
      ...
      console.log("err = ", err); // نمایش خطا در کنسول
    } finally {
  ...
  //صفحه رو جابجا میکنه وقتی روی ادیت میزنه کاربر
  const EditPage = (id) => {
    navigate("/products/" + id);
  }

  //محصول رو اضافه میکنه
  const addProduct = async () => {

    // چک کردن تکراری بودن آی دی نوآمده در لیست
    DataFiltered.map((item, index) =>
      item.id === newProcuctId ? (alert("آیدی وارد شده تکراری است"), setCkeckIdNewProduct(false)) : ""
    )

    // چک کردن صحت فرمت آی دی جدید
    ...
    // چک کردن صحت قیمت جدید  
    else if (isNaN(newProcuctPrice)) {
      alert("قیمت معتبر نیست ")
      setshowModal(false)
    }

    // چک کردن پر بودن تمامی فیلد های فرم   
    ...
    // کد اجرای عمل اضافه کردن محصول
      ...
      try {
        ...
        // بروزرسانی و اضافه کردن محصول به لیست 
        ...
        if (response.status === 200) {
          ...
          // نمایش پیام با رنگ سبز در صورت موفق بودن اولیه
          setTimeout(() => {
            document.getElementById("errorWindow").style.display = "none"
          }, 6000)
          ...
          // بروزرسانی صفحات در صورت نیاز به اضافه شدن صفحه جدید لیست
          if ((Math.ceil(Data.length / row)) > page) setPage(Math.ceil(DataFiltered.length / row))

          // پیغام خطا در صورت ناموفقیت
        } else {
          setMsgApiResult("مشکلی رخ داده است. لطفا مجددا تلاش کنید.")
  ...
  //تغییراتی که کاربر داده است در صفحه ادیت رو اعمال میکنه
  ...
  //توابع زیر برای قسمت سورت کردن استفاده میشه که هر کدوم براساس یه پراپرتی هست
  ...
  //این تابع براساس چیزی که کاربر میخاد یکی از توابع بالا رو انتخاب میکنه
  const sorting = (SortBy) => {
    if (empty !== true) {
      let NullString = "";
      setDataFiltered(NullString)
      setEmpty(true)
      switch (SortBy) {
  ...
  //این میاد سرچ رو انجام میده
  const filtering = (searchIn) => {

    //علت اینکه فارسی گذاشتم این بود که در صورت خالی بودن اسم اینپوت و در غیر اینصورت محتواش رو نشون بده
    //و نیازی نباشه که چند تا استیت جدید تعریف کنم
    switch (searchIn) {
  ...
  return <div className="App">
    <div className='containerr'>
      ...
      {showModal && <div className='moddal'>
        ...
        <div className='moddalFooter'><button className='yesButton' onClick={() => { handleModalRequest() }}>بله</button><button className='noButton' onClick={() => setshowModal(false)}>خیر</button></div>
      </div>}
        ...
        {query === "" ?
            ...
            index < page * row && index + 1 > (page - 1) * row ?
              (<tr>
                ...
                <td className='actionsTd'>
                  <button className='actions deleteAction' onClick={() => {
                    setmodalHeader("https://upload.wikimedia.org/wikipedia/commons/0/0f/Icons8_flat_delete_generic.svg") // تغییر مقدار هدر مدال به عکس حذف
                    setmodalBody("آیا میخواهید این آیتم را حذف کنید؟") // تنظیم متن بدنه مدال
                    setDeleteItemId(item.id); setshowModal(true); setmodalAccept(2)
                  }}>
          ...
          : DataFiltered ? DataFiltered.map((item, index) => (
            <tr>
              ...
              <td>{item.description}</td>
              <td className='actionsTd'>
                <button className='actions deleteAction' onClick={() => {  setmodalHeader("https://upload.wikimedia.org/wikipedia/commons/0/0f/Icons8_flat_delete_generic.svg") // تغییر مقدار هدر مدال به عکس حذف
    setmodalBody("آیا میخواهید این آیتم را حذف کنید؟") // تنظیم متن بدنه مدال
 setDeleteItemId(item.id); setshowModal(true); setmodalAccept(2) }}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7 4a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v2h4a1 1 0 1 1 0 2h-1.069l-.867 12.142A2 2 0 0 1 17.069 22H6.93a2 2 0 0 1-1.995-1.858L4.07 8H3a1 1 0 0 1 0-2h4V4zm2 2h6V4H9v2zM6.074 8l.857 12H17.07l.857-12H6.074z" fill="white" /></svg>
                </button>
        ...
        <tr className='addingInput'>
          ...
          <th>
            <input placeholder=' شناسه' onChange={(e) => {
              setNewProcuctId(e.target.value); const value = e.target.value;
              if (isNaN(+value)) alert("لطفا آیدی را به عدد وارد کنید.");
          ...
          <th>
            <input placeholder='نام' onChange={(e) => { setNewProcuctTitle(e.target.value) }} id="inputNewTitle" />
          </th>

          <th>
            <input placeholder='قیمت' onChange={(e) => {
              setNewProcuctPrice(e.target.value); const value = e.target.value;
              if (isNaN(+value)) alert("لطفا قیمت را به عدد وارد کنید.");
            }} id="inputNewPrice" />
          </th>

          <th>
            <input placeholder='توضیحات' onChange={(e) => { setNewProcuctDescription(e.target.value) }} id="inputNewDescription" />
          </th>

          <th className='addingTh'>
            <button className='actions addAction' onClick={() => {
              setmodalHeader("https://www.freeiconspng.com/thumbs/add-icon-png/add-1-icon--flatastic-1-iconset--custom-icon-design-0.png")
              setmodalBody("آیا میخواهید این آیتم را اضافه کنید؟"); setshowModal(true); setmodalAccept(1)
            }}>
