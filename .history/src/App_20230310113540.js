    ...
    <div className='containerr'>
      ...
      <div className='containCounterPage'>
        ...
        {[...Array(Math.round(DataFiltered.length / row))].map((_, index) => (
          index+1===page ?           <button onClick={() => setPage(page - 1)}>prev</button><button className='pagingButton' style={{ display: index + 1 === page || index + 1 === page - 1 || index + 1 === page + 1 || index + 1 === Math.round(DataFiltered.length / row) || index === 0 ? "flex" : "none" }} onClick={() => setPage(index + 1)}>{index + 1}</button>
          :
          <button className='pagingButton' style={{ display: index + 1 === page || index + 1 === page - 1 || index + 1 === page + 1 || index + 1 === Math.round(DataFiltered.length / row) || index === 0 ? "flex" : "none" }} onClick={() => setPage(index + 1)}>{index + 1}</button>
        ))}
