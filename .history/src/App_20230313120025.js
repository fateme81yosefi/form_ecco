...
function App() {
  ...
  useEffect(() => {
    ...
    console.log(items[0])
    Data.map((data) => data.id === items[0].id ? data.price = items[0].price : "")
    console.log(Data)
  }, []);
  ...
  useEffect(() => {    items = JSON.parse(localStorage.getItem('items'));

    setDataFiltered(Data.map((data) => data.id === items[0].id ? data.price = items[0].price : ""))
  }, [Data]);
