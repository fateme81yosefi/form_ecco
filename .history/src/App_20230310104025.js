...
function App() {
  ...
  const addProduct = async () => {
    if (window.confirm('Do you want to Add it?')) {
      ...
      try {
        const response = await fetch("https://dummyjson.com/products/" + ne, {
          method: "POST",
          body: JSON.stringify(
        ...
        setDataFiltered((x) => [...x, { id: newProcuctId, price: newProcuctPrice, description: newProcuctDescription, category: newProcuctCategory }])
      } catch (err) {
        console.log("err = ", err);
      } finally {
      }
