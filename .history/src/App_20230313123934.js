    ...
    if (items) {
      ...
      DataFiltered.map((i, index) => {
        if (i.id === items[0].id) {
          ...
          // setDataFiltered(Data.splice(index, 1));
          console.log(DataFilData.splice(index, 1)tered)
          // setDataFiltered((x) => [...x, { id: c.id, title: c.title, price: items[0].price, description: c.description, category: c.category }]);
          // setEmpty(!empty)
        }
      })
