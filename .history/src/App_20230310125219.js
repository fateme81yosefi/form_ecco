  ...
  const addProduct = async () => {
    if (window.confirm('Do you want to Add it?')) {
      ...
      try {
        ...
        setEmpty(!empty)
        if (response.status === 200) {
          setNewProcuctCategory("")
          setNewProcuctId("sdf")
          setNewProcuctTitle("sdf")
          setNewProcuctDescription("sdf")
          setNewProcuctPrice("sdf")
          alert("done adding")
    ...
    <div className='containerr'>
      ...
      <table>
        ...
        <tr className='addingInput'>
          ...
          <th><input placeholder='نام' onChange={(e) => { setNewProcuctTitle(e.target.value) }} id="inputNewCategory"/> </th>
          <th><input placeholder='قیمت' onChange={(e) => { setNewProcuctPrice(e.target.value) }} id="inputNewCategory"/></th>
          <th><input placeholder='توضیحات' onChange={(e) => { setNewProcuctDescription(e.target.value) }} id="inputNewCategory"/> </th>
