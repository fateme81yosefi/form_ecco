...
function App() {
  ...
  const addProduct = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if((Math.round(Data.length+1 / row)) > page) setPage(Math.round(DataFiltered.length/ row)+1)
        }
  ...
  useEffect(() => {
    setEmpty(!empty)
    console.log("DataFiltered changed : " , DataFiltered )
    console.log("page : ", page)
    console.log("Math.round(DataFiltered.length / row) : ",Math.round(DataFiltered.length / row))

    console.log("Math.round(Data.length / row) : ",Math.round(Data.length / row))
  ...
  return <div className="App">
    <div className='containerr'>
      ...
      <div className='containCounterPage'>
        ...
        <button onClick={() => page !== Math.round(DataFiltered.length / row) ? setPage(page + 1) : ""}><svg xmlns="http://www.w3.org/2000/svg" width="10.605" height="15.555"><path d="m2.828 15.555 7.777-7.779L2.828 0 0 2.828l4.949 4.948L0 12.727l2.828 2.828z" /></svg></button>
