...
function App() {
  ...
  const applyEdit = () => {
    ...
    DataFiltered.map((i, index) => {
      if (i.id === items[0].id) { setDataFiltered(DataFiltered.splice(index, 1));setDataFiltered((x) => [...x, 
        { id: newProcuctId, title: newProcuctTitle, price: newProcuctPrice, description: newProcuctDescription, category: newProcuctCategory }
      ]) }
    })
    console.log(c.price)
