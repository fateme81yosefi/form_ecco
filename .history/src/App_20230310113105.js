    ...
    <div className='containerr'>
      ...
      <div className='containCounterPage'>
        ...
        {[...Array(Math.round(DataFiltered.length / row))].map((_, index) => (
          <button className='pagingButton' style={{ display:index
             === ||index + 1 === Math.round(DataFiltered.length / row) / 2 || index + 1 === Math.round(DataFiltered.length / row) || index === 0 ? "flex" : "none" }} onClick={() => setPage(index + 1)}>{index + 1}</button>
        ))}
