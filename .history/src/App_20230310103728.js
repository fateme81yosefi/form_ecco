  ...
  const addProduct = async (idProduct) => {
    if (window.confirm('Do you want to Add it?')) {
      ...
      try {
        ...
        setDataFiltered((x)=>[...x,DataFiltered])
      } catch (err) {
        console.log("err = ", err);
      } finally {
      }
    ...
    <div className='containerr'>
      ...
      <table>
        ...
        <tr className='addingInput'>
          ...
          <th><input placeholder='نام' onChange={(e)=>{setNewProcuctCategory(e.target.value)}}/> </th>
          <th><input placeholder='قیمت' onChange={(e)=>{setNewProcuctCategory(e.target.value)}}/></th>
          <th><input placeholder='توضیحات' onChange={(e)=>{setNewProcuctCategory(e.target.value)}}/> </th>
