    ...
    <div className='containerr'>
      ...
      <div className='containCounterPage'>
        {[...Array(Math.round(DataFiltered.length / row))].map((_, index) => (
          <button className='pagingButton' style={{display:index+1 === page || index+1 ===Math.round(DataFiltered.length / row)  || index===0?"flex" }} onClick={() => setPage(index + 1)}>{index + 1}</button>
        ))}
