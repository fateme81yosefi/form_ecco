    ...
    <div className='containerr'>
      ...
      <table>
        ...
        <tr className='addingInput'>
          <th><input placeholder='= شناسه' onChange={(e) => { setNewProcuctId(e.target.value) }} /></th>
      ...
      <div className='containCounterPage'>
        ...
        {[...Array(Math.round(Data.length / row))].map((_, index) => (
          index + 1 === page || index + 1 === Math.round(Data.length / row) || index === 0 ?
            <button className='pagingButton' style={{}} onClick={() => setPage(index + 1)}>{index + 1}</button> : "0"
        ))}
        <button onClick={() => page !== Math.round(Data.length / row) ? setPage(page + 1) : setPage(Math.round(DataFiltered.length / row))}><svg xmlns="http://www.w3.org/2000/svg" width="10.605" height="15.555"><path d="m2.828 15.555 7.777-7.779L2.828 0 0 2.828l4.949 4.948L0 12.727l2.828 2.828z" /></svg></button>
